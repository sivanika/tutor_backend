...
export const getStudentEnrolledSessions = async (req, res) => {
  try {
    const sessions = await Session.find({
    ...
    console.error("FETCH ENROLLED SESSIONS ERROR:", err)
    res.status(500).json({ message: "Failed to fetch enrolled sessions" })
  }
}
